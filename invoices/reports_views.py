...
class ExportClientReportCSVView(ReportBaseView,View):
    def get(self, request, *args, **kwargs):
        ...
        response['Content-Disposition'] = f'attachment; filename="Client_Report_{today}.csv"'

        writer = csv.writer(response)

        # Write header (assumes report_data is a list of dicts or similar)
        if report_data:
            writer.writerow(report_data[0].keys())  # CSV headers
