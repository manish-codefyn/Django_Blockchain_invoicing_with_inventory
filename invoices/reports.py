...
from django.utils import timezone
from decimal import Decimal
from invoices.models import Invoice  # Ensure this import matches your app
...
class InvoiceReports:
    ...
    def generate_monthly_report(year=None, user=None):
        ...
        # Annotate with month using TruncMonth
        ...
        # Build final report with accurate total_amount, paid_amount, overdue_amount
        report = []
        for month_data in monthly_data:
            month = month_data['month']
            month_invoices = queryset.filter(
                date_issued__month=month.month,
                date_issued__year=month.year
            )
            ...
            for invoice in month_invoices:
                ...
                if invoice.status == 'paid':
                    ...
                    overdue += invoice.balance_due

            report.append({
                'month': month,
                'total_invoices': month_data['total_invoices'],
                'total_amount': total,
                'paid_amount': paid,
