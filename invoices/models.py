...
class InvoiceItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.product and not self.tax:
            self.tax = self.product.tax_rate
        super().save(*args, **kwargs)
        
        # Update stock if product is set
        if self.product and self.invoice.status == 'paid':
            ProductStockMovement.objects.create(
                product=self.product,
                movement_type='sale',
                quantity=self.quantity,
                reference=f"Invoice #{self.invoice.invoice_number}",
                created_by=self.invoice.user
            )
